    ...
    <!-- ======= Breadcrumbs Section ======= -->
     <section id="home" class="min-h-100vh flex justify-start items-center">
        <div class="mx-5 md-mx-l5">
            <div>
                <h2 class="black fs-l3 lh-2 md-fs-xl1 md-lh-1 fw-900 ">Doctors Request</h2>

                <%
		session = request.getSession(false);
		String did = request.getParameter("name");
		String sql = "select * from doctors where status2='pending'";
		List<String> lt = Dao.getUser1(sql);
		Iterator<String> itr = lt.iterator();
		%>
			<section id="banner" data-video="images/banner" >
				<div class="inner" >
				<table  class="w3-table w3-blue" style="width:1150px;text-align: center;font-size: 16px;" border="1">
				<tr style="color: blue;"><th>DOCTOR NAME</th><th>DOCTOR ID</th><th>DOCTOR MOBILE NO </th><th>DOCTOR SPECIALIZATION </th><th>DOCTOR EXPERIENCE </th><th>APPROVE</th></tr>
				<%
				while(itr.hasNext()){
					String na = itr.next();
					String id = itr.next();
					...
					String ex = itr.next();
				%>
				<tr style="color: black;"><td><%=na %></td><td><%=id %></td>
				<td><%=mob %></td><td><%=spe %></td>
				<td><%=ex %></td>
				<td><a href="UApprove?em=<%=id%>" class="w3-button w3-black">Approve</a></td></tr>
				<%} %>
				</table>
				</div>
			</section>
