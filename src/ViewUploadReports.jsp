  ...
  background-color: #dddddd;
    ...
    <!-- ======= Breadcrumbs Section ======= -->
        ...
        <br><br><br><br><br><br><br><br><br><br>
            <div>
                <h1 class="black fs-l3 lh-2 md-fs-xl1 md-lh-1 fw-900 ">PATIENT UPLOADED REPORT</h1>
                <br><br>
                <%
		session = request.getSession(false);
		String name = (String) session.getAttribute("name");
		String sql = "select * from uploadfiles where doctorname='"+name+"'";
		List<String> lt = Dao.getDFile(sql);
		Iterator<String> itr = lt.iterator();
 		%>
				<table class="w3-table w3-blue" style="width:1150px;text-align: center;font-size: 16px;" border="1">
				<thead>
				<tr style="color: black;"><th>FILE ID</th><th>PATIENT ID OR MAIL ID</th><th>DOWNLOAD REPORT</th></tr>
				<%
				while(itr.hasNext()){
				%>
				<tr style="color: black;"><td><%=itr.next() %></td><td><%=itr.next() %></td>
				<td><a href="Download1?da=<%=itr.next()%>" class="button-lg bg-indigo-lightest-20 indigo-lightest focus-black fw-300 fs-s3 mr-0 br-l-0">DOWNLOAD</a>
				<%} %>
				</table>
				</div>
        </div>
